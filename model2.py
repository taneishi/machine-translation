...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hidden_dim, nlayers, dropout):
        ...
        self.gru = nn.GRU(emb_dim+hidden_dim, hidden_dim) # emb_dim is current input (target)+hidden_dim is context vector from encoder
        self.out = nn.Linear(emb_dim+hidden_dim*2, output_dim) # hid dim*2 = context vector encoder + prev decoder hidden states 
        self.dropout = nn.Dropout(dropout)
